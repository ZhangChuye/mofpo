...
import torchvision
import logging
from mof.model.common.module_attr_mixin import ModuleAttrMixin

from mof.common.pytorch_util import replace_submodules
...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        if model_name.startswith('resnet'):
            ...
            model[0].stride = (1, 1)
            model[0].padding = (1, 1)
            model[7][0].conv1.stride = (1, 1)
            model[7][0].downsample[0].stride = (1, 1)
            model = torch.nn.Sequential(
                *model,
                torch.nn.Conv2d(feature_dim, feature_dim, kernel_size=3, stride=1, padding=0),
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                # Point-cloud observations are handled by dedicated encoders in policy wrappers.
                # Keep this encoder focused on rgb/low-dim inputs.
                continue
            else:
