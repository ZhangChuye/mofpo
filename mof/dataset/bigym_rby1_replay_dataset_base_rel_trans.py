...
from mof.common.normalize_util import (
    ...
    get_range_symmetric_xyz_normalizer_from_stat,
    get_image_range_normalizer,
    get_image_identity_normalizer,
    get_identity_normalizer_from_stat,
    array_to_stats,
    bigym_action_only_symmetric_xyz_normalizer_from_stat,
)
...
class BigymRBY1ReplayDatasetBaseRelTrans(BaseImageDataset):
    def __init__(
        self,
        shape_meta: dict,
        demos_dir: str,
        ...
        if use_cache:
            ...
            cache_precompute_path = cache_zarr_path + ".base_rel_trans.precomputed.npz"
            ...
            with FileLock(cache_lock_path):
                if os.path.exists(cache_precompute_path):
                    ...
                    if loaded_precomputed:
                        print("Loaded precomputed base-frame/rel-action cache.")
    ...
    def _save_precomputed(self, path: str):
        ...
        print(f"Saved precomputed base-frame/rel-action cache to: {path}")
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        normalizer["action"] = bigym_action_only_symmetric_xyz_normalizer_from_stat(
            stat, share_center_for_arms=True
        )

        action_stat = array_to_stats(self.rel_actions.reshape(-1, self.rel_actions.shape[-1]))
        rel_action_normalizer = bigym_action_only_symmetric_xyz_normalizer_from_stat(
            action_stat, share_center_for_arms=False
        )
        rel_xyz = self.rel_actions[..., [0, 1, 2, 9, 10, 11]]
        rel_xyz_max_abs = max(float(np.max(np.abs(rel_xyz))), 1e-7)
        rel_xyz_scale = 1.0 / rel_xyz_max_abs
        rel_scale = rel_action_normalizer.params_dict["scale"]
        rel_offset = rel_action_normalizer.params_dict["offset"]
        rel_scale.data[[0, 1, 2, 9, 10, 11]] = rel_xyz_scale
        rel_offset.data[[0, 1, 2, 9, 10, 11]] = 0.0
        normalizer["rel_action"] = rel_action_normalizer

        pos_keys = [key for key in self.lowdim_keys if key.endswith("pos") and key != "base_pos"]
        assert 'base_pos' not in pos_keys, "base_pos should not be in pos_keys for shared normalizer"
        pos_normalizer = None
        if pos_keys:
            pos_data = np.concatenate(
                [self.base_frame_lowdim[key] for key in pos_keys],
                axis=0,
            )
            pos_stat = array_to_stats(pos_data)
            pos_normalizer = get_range_symmetric_xyz_normalizer_from_stat(pos_stat)
        ...
        for key in self.lowdim_keys:
            ...
            if key.endswith("pos"):
                this_normalizer = (
                    ...
                    else get_range_symmetric_xyz_normalizer_from_stat(stat)
                )
        ...
        normalizer["base_pos"] = get_range_symmetric_xyz_normalizer_from_stat(base_pos_stat)
    ...
    def _compute_base_rel_trans_action_chunks_indexed(
        self,
        ...
        for start in tqdm.tqdm(range(0, n_samples, batch_size), desc="Precomputing action chunks"):
            ...
            base_pos_t = torch.from_numpy(base_pos_ref[start:end].astype(np.float32))
            base_quat_t = torch.from_numpy(base_quat_ref[start:end].astype(np.float32))
            base_rot = self.quat_to_mat.forward(base_quat_t)

            base_T_inv_single = eye.view(1, 4, 4).repeat(batch_n, 1, 1)
            base_T_inv_single[:, :3, :3] = base_rot
            base_T_inv_single[:, :3, 3] = base_pos_t
            base_T_inv_single = torch.linalg.inv(base_T_inv_single)

            base_T = eye.view(1, 1, 4, 4).repeat(batch_n, horizon, 1, 1)
            base_T[:, :, :3, :3] = base_rot.unsqueeze(1).expand(batch_n, horizon, 3, 3)
            base_T[:, :, :3, 3] = base_pos_t.unsqueeze(1).expand(batch_n, horizon, 3)
            base_T_inv = torch.linalg.inv(base_T)

            left_base_T = base_T_inv @ left_abs_T
            right_base_T = base_T_inv @ right_abs_T
            left_base_xyz = left_base_T[:, :, :3, 3]
            right_base_xyz = right_base_T[:, :, :3, 3]
            left_base_6d = self.sixd_to_mat.inverse(
                left_base_T[:, :, :3, :3].reshape(-1, 3, 3)
            ).reshape(batch_n, horizon, 6)
            right_base_6d = self.sixd_to_mat.inverse(
                right_base_T[:, :, :3, :3].reshape(-1, 3, 3)
            ).reshape(batch_n, horizon, 6)

            left_pos_t = torch.from_numpy(left_pos_ref[start:end].astype(np.float32))
            left_pos_T = eye.view(1, 4, 4).repeat(batch_n, 1, 1)
            left_pos_T[:, :3, 3] = left_pos_t
            left_cur_base_pos = (base_T_inv_single @ left_pos_T)[:, :3, 3]

            right_pos_t = torch.from_numpy(right_pos_ref[start:end].astype(np.float32))
            right_pos_T = eye.view(1, 4, 4).repeat(batch_n, 1, 1)
            right_pos_T[:, :3, 3] = right_pos_t
            right_cur_base_pos = (base_T_inv_single @ right_pos_T)[:, :3, 3]

            rel_action_chunks[start:end] = torch.cat(
                [
                    left_base_xyz - left_cur_base_pos.unsqueeze(1),
                    left_base_6d,
                    right_base_xyz - right_cur_base_pos.unsqueeze(1),
                    right_base_6d,
                    action[:, :, 18:],
                ],
                dim=-1,
    ...
    def _precompute_action_chunks_loop(self):
        ...
        for i in tqdm.tqdm(range(n_samples), desc="Precomputing action chunks"):
            ...
            rel_action_np = _compute_base_rel_trans_action_chunk(
                data_action=action_np,
                base_pos=sample["base_pos"].astype(np.float32),
                base_quat=sample["base_quat"].astype(np.float32),
    ...
    def _precompute_action_chunks(self):
        ...
        base_pos_ref = gather_padded_timestep_from_full_array(
            indices=self.sampler.indices,
            key_first_k=self.key_first_k,
            full_array=np.asarray(self.replay_buffer["base_pos"]).astype(np.float32),
            key="base_pos",
            timestep=base_obs_idx,
        ).astype(np.float32)
        base_quat_ref = gather_padded_timestep_from_full_array(
            indices=self.sampler.indices,
            key_first_k=self.key_first_k,
            full_array=np.asarray(self.replay_buffer["base_quat"]).astype(np.float32),
            key="base_quat",
            timestep=base_obs_idx,
        ).astype(np.float32)
        ...
        rel_action_chunks = self._compute_base_rel_trans_action_chunks_indexed(
...
def _compute_base_rel_trans_action_chunk(
    ...
    base_pos_t = torch.from_numpy(base_pos[base_idx : base_idx + 1])
    base_quat_t = torch.from_numpy(base_quat[base_idx : base_idx + 1])
    base_rot = quat_to_mat.forward(base_quat_t)
    base_T_single = eye.unsqueeze(0)
    base_T_single[:, :3, :3] = base_rot
    base_T_single[:, :3, 3] = base_pos_t
    base_T_inv_single = torch.linalg.inv(base_T_single)

    base_T = eye.unsqueeze(0).repeat(horizon, 1, 1)
    base_T[:, :3, :3] = base_rot.expand(horizon, -1, -1)
    base_T[:, :3, 3] = base_pos_t.expand(horizon, -1)
    base_T_inv = torch.linalg.inv(base_T)
    ...
    left_base_xyz = left_base_T[:, :3, 3]
    right_base_xyz = right_base_T[:, :3, 3]
    left_base_6d = sixd_to_mat.inverse(left_base_T[:, :3, :3])
    right_base_6d = sixd_to_mat.inverse(right_base_T[:, :3, :3])

    left_pos_single = torch.from_numpy(left_pos[base_idx : base_idx + 1])
    left_pos_T = eye.unsqueeze(0)
    left_pos_T[:, :3, 3] = left_pos_single
    left_cur_base_pos = (base_T_inv_single @ left_pos_T)[:, :3, 3]

    right_pos_single = torch.from_numpy(right_pos[base_idx : base_idx + 1])
    right_pos_T = eye.unsqueeze(0)
    right_pos_T[:, :3, 3] = right_pos_single
    right_cur_base_pos = (base_T_inv_single @ right_pos_T)[:, :3, 3]

    left_rel_xyz = left_base_xyz - left_cur_base_pos.expand(horizon, -1)
    right_rel_xyz = right_base_xyz - right_cur_base_pos.expand(horizon, -1)
