        ...
        with JsonLogger(log_path) as json_logger:
            while self.epoch < cfg.training.num_epochs:
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    # Save training RNG state, seed eval deterministically, then restore.
                    ...
                    torch.random.set_rng_state(_rng_state[0])
                    if _rng_state[1] is not None:
                        torch.cuda.set_rng_state_all(_rng_state[1])
                    np.random.set_state(_rng_state[2])
                    random.setstate(_rng_state[3])
                    # log all
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    ...
                    # Top-k selection only fires when the monitor metric is in
                    # this epoch's step_log — i.e., when rollout/eval ran. With
                    # checkpoint_every < rollout_every, intermediate ckpts skip
                    # top-k and only save the last_ckpt (still useful for
                    # preemption recovery). Matches the async workspace's guard.
