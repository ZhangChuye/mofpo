...
# NOTE: do not `import train` here: the checkpoint cfg is already resolved, and spawned env
# workers re-import this module (train.py reopens stdout/stderr at import time).
...
def main():
    ...
    ap.add_argument("--max_steps", type=int, default=None, help="TESTING ONLY: override episode length (paper values are baked in the config)")
    a = ap.parse_args()
...
if __name__ == "__main__":
    main()  # required: the vector env uses the 'spawn' start method, which re-imports this file
